import './TermsOfUseSignUpPanel.css'

import { useRef, useEffect, useState } from 'react';

import AcceptTermsCheckBox from '../../TermsOfUseParts/accept-terms-form/AcceptTermsCheckBox';

const TermsOfUseSignUpPanel = (props) => {

  const termsAccepted = props.termsAccepted
  const setTermsAccepted = props.setTermsAccepted

  const termsScrollRef = useRef()
  const [termsScrolled, setTermScrolled] = useState(false)
  ...
  useEffect(() => {
    ...
    if (termsElement) {
      termsElement.addEventListener("scroll", monitorUserScroll);
    }

    return () => {
      termsElement.removeEventListener("scroll", monitorUserScroll);
    }

  }, [])

  return (
    <div className="terms-of-use-signup-container">
      <div className="terms-of-use-title-container">
        Please Read Our Terms of Use
      </div>
      <div>
        <div className='terms-of-use-text-container' ref={termsScrollRef}>
      ...
      <AcceptTermsCheckBox termsScrolled={termsScrolled} termsAccepted={termsAccepted} setTermsAccepted={setTermsAccepted}/>
    </div>
  )
}

export default TermsOfUseSignUpPanel;